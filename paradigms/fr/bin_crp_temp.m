function lag_crps = bin_crp_temp(recalls_matrix, subjects, list_length, ...
...
function subj_crp = crp_for_subj(recalls, from_mask_rec, to_mask_rec, ...
  ...
  bin_subj_possibles = zeros(size(bins));
  bin_subj_actuals = zeros(size(bins));
  ...
  for i = 1:num_trials
    ...
    [trial_actuals, trial_possibles] = conditional_transitions_strict(...
					recalls(i, :), ...
    	                                from_mask_rec(i, :), to_mask_rec(i, :), ...
					@lag, @possible_transitions, ...
			                step, params);    
    % SMP: we need to bin at the level of individual transitions
    % actual_transitions = [actual_transitions, trial_actuals];
    % poss_transitions = [poss_transitions, catcell(trial_possibles)];
        
    bin_trial_actuals = histc(trial_actuals, bins);
    bin_subj_actuals = bin_subj_actuals + bin_trial_actuals;
    
    for j=1:length(trial_possibles)
       if length(trial_possibles{j}) > 0
           bin_transit_possibles = histc(trial_possibles{j}, bins);
           % for a given transit a bin is either available or unavailable
           bin_transit_possibles = logical(bin_transit_possibles);
           bin_subj_possibles = bin_subj_possibles + bin_transit_possibles; 
       end
    end
  ...
  subj_crp = bin_subj_actuals ./ bin_subj_possibles;
