function sim_facts = sim_fact_cat(recalls_itemnos, pres_itemnos, ...
                                    sim_mat, pres_catlabels, cat_type, ...
                                    subjects, rec_mask, pres_mask)
%SIM_FACT_CAT   Similarity-based clustering factor conditional on category.
%
%  Determine whether recall is organized by some measure that is
%  continuous and varies between item pairs, for example an
%  estimate of semantic similarity. The measure is conditional on
%  the type of category transition (within or between category).
%
%  sim_facts = sim_fact_cat(recalls_itemnos, pres_itemnos, sim_mat, 
%                           pres_catlabels, cat_type, 
%                           subjects, rec_mask, pres_mask)
%
%  INPUTS
...
%
%  pres_catlabels - [lists x serial position] numeric array
%      Category labels of presented items. Each category label can
%      be any numeric value, as long as it is unique to that category.
%
%  cat_type - integer
%      Type of transitions to include in the analysis:
%      1 - Only include within-category transitions.
%      2 - Only include between-category transitions.
%
...
sim_facts = apply_by_index(@sim_fact_for_subj, subjects, 1, ...
                            {recalls_itemnos, pres_itemnos, pres_catlabels, ...
                             rec_mask, pres_mask}, cat_type, sim_mat);
%endfunction
...
step = 1;
params = struct('sim_mat', sim_mat, 'cat_type', cat_type);
...
function ps = possible_sim_transitions(item, prior_items, item_sim, params)
  ...
  curr_cat = params.pres_catlabels(params.trial_pres_itemnos == item);
  if params.cat_type == 1
      pres_mask = (params.pres_catlabels == curr_cat) & params.trial_pres_mask;
  else
      pres_mask = (params.pres_catlabels ~= curr_cat) & params.trial_pres_mask;
  end
  unmasked_pres_items = params.trial_pres_itemnos(pres_mask);
